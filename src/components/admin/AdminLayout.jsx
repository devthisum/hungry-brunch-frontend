...
import {
  ...
  Clock, Menu, X, LogOut, ChevronRight, CalendarCheck, Tag, QrCode
} from 'lucide-react';
...
export default function AdminLayout({ children }) {
  ...
  const [newReservations, setNewRes]      = useState(0);
  const location  = useLocation();
  const navigate  = useNavigate();
  const { admin, logout } = useAuth();

  useEffect(() => {
    const fetch = () => reservationAPI.getCounts().then(r => setNewRes(r.data.data.new)).catch(() => {});
    fetch();
  ...
  const navItems = [
    ...
    { to: '/admin/promotions',   Icon: Tag,             label: 'Special Offers' },
    { to: '/admin/reviews',      Icon: Star,            label: 'Reviews' },
    { to: '/admin/gallery',      Icon: Image,           label: 'Gallery' },
    { to: '/admin/hours',        Icon: Clock,           label: 'Opening Hours' },
    { to: '/admin/qrcode',       Icon: QrCode,          label: 'QR Code Menu' },
  ];
          ...
          return (
            <Link key={to} to={to} onClick={() => setSidebarOpen(false)}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all group ${
                active ? 'bg-gradient-to-r from-accent/20 to-gold/10 text-gold border border-accent/20'
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        <header className="sticky top-0 z-30 glass-dark border-b border-white/5 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden text-cream/60 hover:text-cream p-1">
              <Menu size={20} />
            </button>
            <div>
          ...
          <Link to="/" className="text-xs text-cream/40 hover:text-cream transition-colors">View Site →</Link>
        </header>
